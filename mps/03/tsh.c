...
/*void sigchld_handler(int sig) 
{
  ...
  return;*/
  void sigchld_handler(int sig)  //Terminated or stopped child 
  {
    pid_t pid;
    int status;
    while ((pid = waitpid(-1, &status, WUNTRACED | WNOHANG)) > 0) {
      //this while lopp checks the process and see if it has been stopped completely
      //WUNTRACED|WNOHANG :suspend execution of the calling process until a child in the wait
      //set terminates or stops, and then return the pid of the stopped or terminated child 
      if (WIFSTOPPED(status)) //caught
	{
	...
	if (WIFSIGNALED(status)) {
	  //if the child process terminated because of a signal that was not caught

	  if (WTERMSIG(status) == SIGINT) {
	    //if the signal that cause the process to terminate is Ctrl+C 
	    int jid=pid2jid(pid);
	    if (jid!=0) {
	      //interrupted
